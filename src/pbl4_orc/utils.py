...
def consecutive(data, mode="first", stepsize=1):
    ...
    if mode == "first":
        result = [elem[0] for elem in group]
    elif mode == "last":
        result = [elem[-1] for elem in group]
    return result
...
class BeamEntry:
    ...
    def __init__(self):
        ...
        self.simplified = True  # To run simplify label
...
def applyLM(parentBeam, childBeam, classes, lm):
    """calculate LM score of child beam by
...
def fast_simplify_label(labeling, c, blankIdx=0):
    ...
    # Cases that might still require simplifying
    else:
        newLabeling = labeling + (c,)
...
def ctcBeamSearch(mat, classes, ignore_idx, lm, beamWidth=25, dict_list=[]):
    ...
    for t in range(maxT):
        ...
        for labeling in bestLabelings:
            ...
            # char_highscore = np.argpartition(mat[t, :], -5)[-5:]
            # run through 5 highest probability
            ...
            for c in char_highscore:
                ...
                # apply LM
                # applyLM(curr.entries[labeling], curr.entries[newLabeling], classes, lm) # noqa: E501
