...
class Batch_Balanced_Dataset(object):
    def __init__(self, opt):
        ...
        the 50% of the batch is filled with MJ
        and the other 50% of the batch is filled with ST.
        """
        ...
        for selected_d, batch_ratio_d in zip(opt.select_data, opt.batch_ratio):
            ...
            ex) opt.total_data_usage_ratio = 1 indicates 100% usage,
            and 0.2 indicates 20% usage.
...
class OCRDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # We only train and evaluate on alphanumerics
        # (or pre-defined character set in train.py)
        out_of_char = f"[^{self.opt.character}]"
