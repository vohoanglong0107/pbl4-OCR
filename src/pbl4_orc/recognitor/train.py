...
from .labelconverter import CTCLabelConverter, AttnLabelConverter, Averager
...
def train(opt, show_number=2):
    ...
    for _ in tqdm(range(start_iter, opt.num_iter)):
        ...
        if (i % opt.valInterval == 0) and (i != 0):
            ...
            # for log
            with open(
                f"./saved_models/{opt.experiment_name}/log_train.txt", "a"
            ) as log:
                model.eval()
                with torch.no_grad():
                    (
                ...
                loss_log = (
                    f"[{i}/{opt.num_iter}] "
                    f"Train loss: {loss_avg.val():0.5f}, "
                    f"Valid loss: {valid_loss:0.5f}, "
                ...
                current_model_log = (
                    ...
                    f'{"Current_norm_ED":17s}: {current_norm_ED:0.4f}'
                )
                ...
                best_model_log = (
                    ...
                    f'{"Best_norm_ED":17s}: {best_norm_ED:0.4f}'
                )
